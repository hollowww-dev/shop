    ...
    return (
        <Card className="[&:not(:first-child)]:mt-2">
            <CardContent className="relative py-2 flex flex-row gap-2">
                ...
                />
                <Image src={entry.image!} alt={entry.name} width={60} height={60} className="self-start rounded-full p-1 border border-border" unoptimized />
        ...
        <Sheet onOpenChange={handleCartClick} open={shouldDisplayCart}>
            <SheetTrigger onClick={() => dismiss()} asChild>
                <Button variant="ghost" className="p-2 text-2xl relative">
                    <AiOutlineShopping />
                    {cartCount ? (
                        <div className="flex justify-center items-center absolute bottom-[2px] w-[17px] h-[17px] right-0 rounded-full text-[10px] bg-background border">
